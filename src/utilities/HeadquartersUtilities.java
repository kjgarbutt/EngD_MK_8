...
public class HeadquartersUtilities {
	...
	public static ArrayList <ArrayList <AidParcel>> gridDistribution (ArrayList <AidParcel> parcels, GeomVectorField space, int targetSize){
		...
		ArrayList <ArrayList <AidParcel>> rounds = new ArrayList <ArrayList<AidParcel>> (); // for safety??
		...
		for(int i = 0; i < (int)Math.ceil(roundsPerSide * roundsPerSide)+1; i++){
			rounds.add(new ArrayList <AidParcel> ());
		}
		...
		for(int i = 0; i < rounds.size(); i++){
			if(rounds.get(i).size() > 0)
				return new ArrayList <ArrayList<AidParcel>> (rounds.subList(i, rounds.size()));
		}
		return new ArrayList <ArrayList<AidParcel>> ();
	}
